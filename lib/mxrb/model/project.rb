...
module Mxrb
  module Model
    class Project
      ...
      # Reports the explicit removals of the supported target strategy without
      # modifying the project. An empty report is not a compatibility verdict.
      def migration_losses(version)
        auditor = StudioCompatibility::LossAudit.new(target_version: version)
        all_units.flat_map do |unit|
          auditor.audit(parse_bson(unit), unit_id: unit.fetch('UnitID').to_s)
        end.freeze
      end
      ...
      def migrate_to!(version)
        ...
        raise ArgumentError, "project is opened read-only; reopen with readonly: false" if @mpr.readonly?

        if v == StudioCompatibility::LossAudit::TARGET_VERSION && mendix_version != v
          losses = migration_losses(v)
          unless losses.empty?
            raise UnsupportedVersion,
                  "migration to #{v} would remove #{losses.size} properties or settings parts; " \
                  'inspect migration_losses(version); semantic equivalence is not established'
          end
        end

        rewrite_for_version!(v, definition.merge(version: v))
        refresh!
        self
      end
      ...
      def rewrite_for_version!(version, definition)
        path = @mpr.path
        @mpr.close
        Writer.new(path, definition).write!
        @mpr = IO::MprFile.open(path, readonly: false)
        @mpr.update_version!(version)
        @mpr.ensure_storage_for_version!(version)
      end
    end
  end
end
